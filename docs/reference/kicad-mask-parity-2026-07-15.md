...
The reproducible fixtures, commands, Gerbers, DRC reports, parsed measurements, and hashes are preserved under `docs/reference/fixtures/kicad-mask-parity-10.0.3/`. The bundle has 88 files: 87 artifacts covered by `hashes.sha256`, plus the manifest itself. The manifest SHA-256 is `22B8B15D93D1E3472B7222E9A8FB55D7908D73DC5ABAB1BA69084C62A96BDF14`. `commands.ps1` records the successful export/API commands, and `measurements.json` contains parsed aperture definitions and flash coordinates.
