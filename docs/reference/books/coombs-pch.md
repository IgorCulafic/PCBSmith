...
| item | verdict | note |
|------|---------|------|
| §1.1 acid-trap mechanism + "largely obsolete" claim (37.7.2.2 / cited 37.6) | MISMATCH | The 37.7.2.2 quotes ("the narrower and deeper the channel, the slower the flow"; "a sharp 90° angle has a slower-etching section in the inside edge of the bend") are verbatim-correct (`c0059-part0048_split_000.html.txt` lines 448-450). The second bullet's quote ("occurs when etch action is more rapid at the edges..."; "Modern etchers with reduced pooling of etchant have reduced this problem.") is also verbatim-correct (same file, lines 74-76) but its **WHERE locator is wrong**: it sits in unlabeled text right after §37.2.5.7 ("Etching Procedures for Plated-Metal Resists") and before §37.3 ("RESIST REMOVAL") — not under §37.6, which is confirmed (same file, lines 396-424) to be "METALS OTHER THAN COPPER" (aluminum/nickel/stainless steel/silver etching), an unrelated topic. Correct locator is within §37.2 (unlabeled paragraph, immediately following 37.2.5.7), not 37.6. |
| §1.2 fine-line floor = resist + foil thickness (37.7.4.2) | VERIFIED | Exact match: "for 1.2-mil dry-film resist over 1-oz copper foil, the total thickness is 2.6 mils" and "...a 2.6-mil resist line would be 2.6 mils at the trace bottom and would have a 1.5-mil etched top reduction, leaving only a 1.1-mil top surface" (`c0059-part0048_split_000.html.txt` line 478). Numbers and 0.066 mm conversion check out. |
| §4.1 milled-slot floor / router diameter (38.2.1, 38.6.6) | VERIFIED | 38.2.1 counterclockwise-outline/clockwise-cutout rule confirmed verbatim (`c0061-part0049.html.txt` line 98). 38.6.6 confirms "maximum spindle speed is 100,000 RPM... required for diameters being smaller than 0.7 mm" and the 3.175 mm shank-diameter example (same file, lines 282-284). The notes' "0.7 mm comfortable floor / 0.6 mm marginal" framing is the note-writer's engineering interpretation of the RPM-ceiling fact, not a verbatim book statement — flagged as reasonable inference, not a misquote. |
| §5.1 plating throwing power vs. aspect ratio (33.3.1.2) | VERIFIED | Exact match: "100 percent TP) for a 3:1 aspect ratio hole and may be only 1/3 of the thickness on the surface (33 percent TP) for a 15:1 aspect ratio hole. In the latter case to achieve a thickness of 1.0 mil in the middle of the hole would result in plating 3.0 mils on the surface" (`c0054-part0044.html.txt` line 56). |
| §3.4 via-in-pad fill + cap-plating requirement (36.6, 53.11.2.5.11) | VERIFIED | 36.6.1 confirms "vias should be protected from both sides, not only one side... a ring void of copper results at the barrel-plug interface that is subject to corrosion" (`c0058-part0047.html.txt` line 387). 53.11.2.5.11 confirms the copper wrap "needs to extend at least 25 μm from the edge of the drilled via" (same file, line 546). |
| §2.1 double-sided reflow side assignment + no numeric g/mm gate (43.3.2, 43.3.3.2.1) | VERIFIED | 43.3.2 quote confirmed verbatim: "larger components should be placed on the side of the circuit board that is soldered last. If placed on the first side to be soldered, large packages may fall off... unless those devices are secured in place with an adhesive or 'staking' compound" (`c0068-part0056_split_000.html.txt` line 270). 43.3.3.2.1 confirms "the package weight exceeds the surface tension force of the molten solder" (line 372) and the run-out/bleeding language (line 376). Searched the full chapter text for "g/in", "g/mm", and "gram" — no numeric mass/area gate appears anywhere; the industry 30 g/in² heuristic is indeed absent from this book, as claimed. |
| §3.3 trace-into-pad ≤60% width ratio (21.10) | VERIFIED | Exact match, same passage as the acid-trap §1.1 quote: "Trace entry into pads should not leave an acute angle which may cause fabrication issues" and "Trace width should be no more than approximately 60 percent of the pad size it is entering for heating reasons during assembly" (`c0036-part0030.html.txt` lines 316-318). |
| §3.2 solder-mask dam width (36.3, 36.4.4.2) | VERIFIED | 36.3.2 confirms "webs are frequently as narrow as 0.003 in. and can be as narrow as 0.001 in." requiring photoimageable mask (`c0058-part0047.html.txt` line 78). 36.4.4.2 confirms "width of these dams is frequently in the 2.5 to 3.0 mil range, with some designs requiring dams as narrow as 1.0 to 1.5 mils wide" (same file, line 126). |

8/8 content-verified (all quoted claims and numeric thresholds confirmed verbatim in the cache); 1 locator mismatch found (§1.1 second bullet cites WHERE 37.6, which is actually "Metals Other Than Copper" — the real location is an unlabeled paragraph within §37.2, right after §37.2.5.7). No UNFINDABLE items.
